...
class WeatherDateController {
  ...
  async store ({ request, response, params }) {
    try {  
        ...
        // console.log(timezone_station[0]["timezone"])
       ...
       // console.log(data_fuso.utc()['_d'])
        data.moment = data_fuso.utc()['_d']
  ...
  async show ({ params, request, response, view }) {
    try {
        ...
        // percorre a data de cada dado para adequar ao fuso horário da estação
        for (var i = 0; i < data.length; i++) { 
          var date = new Date(data[i].moment);
          data[i].moment = moment.tz(date, timezone_station[0]["timezone"])
        }
